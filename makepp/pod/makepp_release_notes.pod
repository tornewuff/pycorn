...
=for vc $Id: makepp_release_notes.pod,v 1.57 2009/02/09 22:07:39 pfeiffer Exp $
...
option C<-A> or C<--args-file> is now consistently available on all commands.
...
New option C<--stop-after-loading> (or C<--stop>) gives makepp a headstart
...
New scanner C<esqlc_compilation> for embedded SQL C.  You must supply the
rules, however.
...
New options C<--rm-stale>, C<--sandbox> and C<--dont-read>, for sandbox
...
that if you change a file and revert, then you can have makepp drop back to
