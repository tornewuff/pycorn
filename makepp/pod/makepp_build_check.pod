...
=for vc $Id: makepp_build_check.pod,v 1.10 2009/02/09 22:07:39 pfeiffer Exp $

=head1 DESCRIPTION

L<B<a>rchitecture_independent|/architecture_independent>,E<nbsp>
L<B<e>xact_match|/exact_match>,E<nbsp>
L<B<i>gnore_action|/ignore_action>,E<nbsp>
L<B<o>nly_action|/only_action>,E<nbsp>
L<B<t>arget_newer|/target_newer>
...
using C<:build_check> modifier on the rule (see
L<makepp_rules/build_check>), or for all rules in a makefile by using
the C<build_check> statement (see L<makepp_statements/build_check>, or
for all makefiles at once using the C<-m> or C<--build-check-method>
command line option (see L<makepp_command/--build-check-method>).

=head2 Build check methods included in the distribution

At present, there are four build check methods included in the
distribution.
...
The C<target_newer> method isn't as safe as the C<exact_match> method
because it won't trigger a rebuild if you change the build command, or
if you replace a file with an older version.  Sometimes also it can get
confused if clocks are not properly synchronized.  For example, if a
file somehow gets a date of June 4, 2048, then between now and 2048,
every file that depends on that file will be rebuilt even though the
file doesn't change.  Also switching to a different architecture won't
trigger a rebuild.

But there are a few cases where you may
want to use the C<target_newer> method:
...
have commands in your makefile like this:
    ...
    x.o : x.c
 	rsh $(REMOTE_MACHINE) cc $< -o $@

you might want makepp not to force a rebuild if C<$(REMOTE_MACHINE)>
changes.  You could modify the C<exact_match> method so it knows about
rsh commands and ignores the machine name.
