...
=for vc $Id: makepp_repositories.pod,v 1.20 2010/02/24 23:30:30 pfeiffer Exp $
...
functionality to the C<VPATH> variable in some versions of make, but (unlike
C<VPATH>) you do not have to do anything special to your makefile to get them
to work.

Repositories are specified with the C<-R> or C<--repository> command line
option (see L<makepp_command/-R>) or with the C<repository> statement (see
L<makepp_statements/repository>) in the makefile.  Note that if you have a
habit of calling makepp in different subdirectories of your build tree, it is
easy to accidentally reimport a repository somewhere else.  As a safeguard
against this, if you use F<RootMakeppfile>, makepp will refuse to start if it
finds one above or below where it would be imported.
...
(C<-F> changes to that directory before loading the makefile.  See
L<makepp_command/-F> for details.  You must specify C<-R> before C<-F>.)  Note
that this example puts the new build tree inside the repository.  That will
not work if you use a F<RootMakeppfile> because makepp safeguards against
...
and, because options are seen before makefiles are read, you can call just
