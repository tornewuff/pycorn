...
=for vc $Id: makepp_build_cache.pod,v 1.22 2009/02/09 22:07:39 pfeiffer Exp $
...
C<&>-prefix as follows for the example of C<create>:
    ...
    perl { use Mpp::BuildCacheControl } # It's a Perl module, so use intead of include.
...
=item create [option ...] path/to/cache ...
...
expensive, it is sugested doing this only at night.
...
=item show [option ...] /path/to/cache ...
...
=item S, sym-links (only for grouped build caches)
...
In the long verbose form you get the file access (read) time, the modification
 ...
 to the cached file).  In the short standard form you get only one of the
 three times in three separate columns:
...
=item stats [option ...] /path/to/cache ...
