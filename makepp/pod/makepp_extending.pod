...
=for vc $Id: makepp_extending.pod,v 1.13 2007/04/27 20:06:18 pfeiffer Exp $
...
variables are stored in the C<global::> package.  See L<Makefile
...
The block operator C<Makecmds::frame> is followed by a single letter option
...
details).  The class's name must be prefixed with C<Signature::>, so in
this case our class should be called C<Signature::shared_object>.  We
have to create a file called F<shared_object.pm> and put it into a
F<Signature> directory somewhere in the perl include path; the easiest
place might be in the F<Signature> directory in the makepp installation
(e.g., F</usr/local/share/makepp/Signature> or wherever you installed
it).

For precise details about what has to go in this class, you should look
carefully through the file F<Signature.pm> and probably also
F<Signature/exact_match.pm> in the makepp distribution.  But in our
...
to inherit from C<Signature::c_compilation_md5>, which is the signature
method that is usually chosen when makepp recognizes a link command.

So the file F<Signature/shared_object.pm> might contain the following:

    use strict;
    package Signature::shared_object;
    use Signature::c_compilation_md5;
    our @ISA = qw(Signature::c_compilation_md5); # Indicate inheritance.
...
F<Signature/shared_object.pm>), but it will only work with GNU binutils
