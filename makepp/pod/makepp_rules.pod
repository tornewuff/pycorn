...
=for vc $Id: makepp_rules.pod,v 1.26 2010/02/24 23:29:45 pfeiffer Exp $
...
Rule varies depending on compiler:
    ...
    %.o : %.c
 	: include %.d : signature c_compilation_md5
 	gcc -MD -c ...
 
    %.o : %.c
 	: include %.u : signature c_compilation_md5 # IBM uses a different suffix
 	xlc -M -c ...
...
WARNING: This is efficient, but inherently unreliable.  The dependency file
gets produced by the very rule for which it is a dependency.
...
If you build in your repositories, makepp will pick up the depency file from
the 1st repository which contains one.  This is unlike other files, where it
takes the 1st with the expected signature.  This is better than for build
