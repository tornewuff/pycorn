...
=for vc $Id: makepp_rules.pod,v 1.32 2010/11/17 21:35:52 pfeiffer Exp $

=head1 DESCRIPTION

=for genindex '$str =~ s/^://; $str =~ s/^(?!\w)/&/' '(?:[-@&]|:?\w+)' makepp_rules.pod
...
=for comment This one must be 1st, because all single symbols get html id "_",
...
This could not be acheived by replacing C<*> with C<%>, because the latter is
