# $Id: Verilog.pm,v 1.17 2006/11/18 16:45:06 pfeiffer Exp $
...
sub xscan_line
{
    ...
    my $state = $finfo->{"Scanner::Verilog state"} ||= {};
...
sub xscan_file {
  ...
  my $absname=FileInfo::absolute_filename( $finfo );
  ...
  while( defined($_ = $self->xscan_line(@scan_Ctx)) )
  {
	...
	if($map->{$module}) {
	  my $n2 = FileInfo::absolute_filename( $map->{$module}[0] );
