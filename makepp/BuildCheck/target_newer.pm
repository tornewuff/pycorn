# $Id: target_newer.pm,v 1.8 2006/11/22 22:13:15 pfeiffer Exp $
use strict;
package BuildCheck::target_newer;

use BuildCheck;
our @ISA = qw(BuildCheck);
...
sub build_check {
  ...
  warn "signature methods for the target_newer build_check method are ignored\n"
    if $sig_method != $Signature::signature &&
    !$already_warned_about_signature_methods++;

  if( !FileInfo::file_exists $tinfo ) {	# If there's no target, then we need to
				# rebuild.
...
#
  my $target_mtime = FileInfo::build_info_string( $rep_entry, 'SIGNATURE' );
  $target_mtime =~ s/,.*//;	# Strip out the file size, leaving the file
                                # date.
  foreach my $dep (@$sorted_dependencies) {
    return 1 if FileInfo::file_mtime( $dep ) > $target_mtime;
  }
  return undef;
}
...
sub changed_dependencies {
  ...
  foreach (@dependencies) {
    next if FileInfo::assume_unchanged( $_ );
    push @changed, $_
      if $_->{ASSUME_CHANGED} ||
	FileInfo::file_mtime( $_ ) > ($mtime ||= FileInfo::file_mtime $target);
  }
